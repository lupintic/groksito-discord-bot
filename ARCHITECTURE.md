...
- `llm_input.py`: Sole authoritative builder of `initial_input` for the Responses API. Always exactly **one** system message containing the fixed `SYSTEM_PROMPT`. All dynamic context on addressed turns — high-priority `[R:]` referenced message + reply-chain ancestors, plus a compact server emoji header (now top ~8 most-used for the current guild only, with short descs) — is folded as a short prefix at the start of the user message. Emote data is strictly per-guild using live Discord + persisted usage counts. Pure image/video creation turns intentionally skip the variable emote block (stable minimal input for cross-guild consistency and language matching); normal chat keeps full emote injection. This design guarantees a consistent prefix under the per-user `prompt_cache_key`, maximizing KV cache hits on the stable `SYSTEM_PROMPT` block while keeping the information visible to the model.
