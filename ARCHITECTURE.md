...
### Context Management
- `context/core.py`: Short-term per-channel history buffers (always updated on incoming messages). Used for get_channel_context / get_recent_context tools and (legacy/disabled) summarization.
- `context/context_summarizer.py`: Summarizer used on-demand by the get_recent_context tool handler (no longer called for pre-injection).
- Recent context is now on-demand via tool (no automatic pre-injection since #19); "smart mode" and legacy flags remain for the tool path and config surface.
...
### Sandbox
- `sandbox.py`: Implements safe execution environments (Docker-based) for privileged tools such as code execution and browser automation. These tools are **never** offered in normal chat ΓÇö they are only injected when an approved skill's `allowed_tools` explicitly requests them.
...
1. `client.py` receives a message ΓåÆ guild/rate-limit checks ΓåÆ always updates short-term context buffers ΓåÆ hands off to `conversation.py`.
...
**Performance notes (Ticket #5 + #19):** The dominant latency source on normal @mentions was the `summarize_recent_conversation` pre-call (extra Responses roundtrip) + injected recent summary block. Gating (#5/#18) + full removal of pre-injection (#19) eliminated it: recent context is now available *only* as an on-demand tool (`get_recent_context`, offered via light decision tools on addressed turns). Simple @mentions pay zero extra pre-LLM cost for context; the model calls the tool (adding a roundtrip) *only* when it reasons that recent summary is required. Native long context (via previous_response_id), [R:] refs, and reply chains cover most coherence. Raw pre-fallbacks also removed. This is the target "on-demand" state.
...
**Vision**: Evolve Groksito toward a more agentic architecture similar to how Grok operates on the web ΓÇö with less hardcoded logic and more native reasoning by the model itself.
...
### Core Goals
- **Letting Grok reason more natively**: The model (not Python) decides high-level strategy on most turns ΓÇö when to answer directly from knowledge, when fresh data is required, when to pull recent context, when (and how) to activate or create a skill, and which interaction primitive to use.
...
- **Inside an activated turn ΓÇö native reasoning dominates**:
  ...
  - Recent conversation context and channel state are strictly on-demand tools (get_recent_context) rather than pre-injected (#19 completed the migration; no automatic summaries or raw recent blocks in the initial prompt).
  - Key Discord interaction primitives are exposed as first-class, well-described native tools (generalized reply/send under the safety envelope ΓÇö see #13) so the model can choose the right conversational action instead of having orchestration hardcoded in Python.
...
- #10 Reduce Pre-filtering on Normal @mentions (Phase 1 continuation) ΓÇö broaden when light decision tools are offered, relax classification gating where it unnecessarily limits the model.
- #19 Make Recent Context Available as an On-Demand Tool ΓÇö removed automatic pre-summarization / raw recent injection from llm_input and llm paths; recent context summaries are generated exclusively when the model calls get_recent_context (light decision tools ensure it is visible on addressed turns). (Prior roadmap listed similar goal as #11.)
- #12 Improve Tool Descriptions for Better Native Reasoning ΓÇö high-leverage refinements to native search and decision tool schemas so Grok makes higher-quality choices.
- #13 Expose Key Discord Actions as Native Tools ΓÇö reply, send, and related primitives become clean tools the model can invoke naturally (while the activation/safety layer still guarantees correct delivery).
- #14 Simplify and Deprecate Heavy Classification & Decision Logic ΓÇö systematically reduce the custom keyword lists, `classify_query_context_need`, and heuristic pre-decisions that currently control too much behavior.
- #15 Add Better Observability for Tool Decisions ΓÇö logging/metrics so we can see what the model is choosing and why (search vs direct, skill use, etc.).
- #16 Clean Up Deprecated Code After Agentic Migration ΓÇö final removal of the old classification/decision scaffolding once the native paths are proven.
