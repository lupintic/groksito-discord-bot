...
def register_slash_commands(
    ...
    """
    # /mislimites — shows remaining requests for the current user (rate limit info)
    @tree.command(
        name="mislimites", description="Muestra cuántas requests te quedan en este minuto"
    )
    ...
    # /steamchart — optional juegos (comma-separated). Falls back to a sensible default list.
    ...
    async def steamchart(interaction: discord.Interaction, juegos: Optional[str] = None):
        ...
        if not resolved_games:
            await interaction.followup.send(
                "No pude reconocer ningún juego con ese nombre. "
        ...
        # Build one embed per game — identical structure to /stmchr
        embeds = []
        for g in games_data:
    ...
    # /stmchr — always shows the fixed list of 8 games (no parameters needed).
    ...
    @tree.command(
        name="stmchr", description="Black Desert, PoE2, GW2, Lost Ark, Crimson Desert y más"
    )
    ...
    # /topgames — shows the real-time Top 10 (or so) games by current players
    ...
    @tree.command(
        name="topgames",
        description="Top 10 juegos con más jugadores actuales en Steam (de steamcharts.com/top)"
    )
...
async def ensure_discord_connected(conversational: bool = True) -> "discord.Client":
    ...
    async def on_ready():
        logger.info(f"✅ Groksito connected as {_discord_client.user} (ID: {_discord_client.user.id})")
        ...
        if _ALLOWED_GUILD_IDS:
            logger.info(f"[SECURITY] Guild whitelist ACTIVE — {len(_ALLOWED_GUILD_IDS)} allowed guild(s)")
        else:
            logger.warning("[SECURITY] No ALLOWED_GUILD_IDS set — bot will respond in ANY server.")
    ...
    async def on_message(message: discord.Message):
        ...
        try:
            ...
            # Always track context (for optional summarization + get_channel_context tool + legacy)
            # Also capture images and links so the Recent Conversation Context summarizer
            ...
            if is_mentioned:
                ...
                # Plain message with no mention → ignore (no reply reference)
                return
            ...
            if not can_use:
                await _safe_reply(message, "Tranquilo campeón, ya usaste tus 6 requests este minuto.", mention_author=False)
                return
            ...
            # All injection decided inside llm_input.build_responses_input ([R:] on bot replies + mention-in-reply cases).
