...
class GroksitoSettings(BaseSettings):
    ...
    # -------------------------------------------------------------------------
    # Recent Conversation Context (lightweight on-demand summary for normal chat)
    # Only generated and injected when Groksito is directly mentioned or replied to
    # (or the user explicitly asks about prior conversation). This is separate from
    # the old proactive older-history summarization (which is disabled by default).
    # -------------------------------------------------------------------------
    enable_recent_context_summary: bool = Field(
        default=True,
        description="Enable the recent conversation context summary feature. When the bot is mentioned or replied to, a short high-quality summary of recent channel messages is generated using a dedicated prompt and injected as a system message for better coherence.",
    )
    ...
    recent_context_message_limit: int = Field(
        default=20,
        description="Maximum number of recent messages to consider when building the recent conversation context summary.",
    )
    recent_context_max_tokens: int = Field(
        default=400,
        description="Target maximum size (in tokens) for the generated recent conversation context summary. The dedicated summarization prompt aims for concise, high-signal output in the 250-450 token range.",
    )
