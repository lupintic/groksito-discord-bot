...
async def _resolve_referenced_and_activation(
    ...
    - Reply to *another user* → only activate on strong directed signals
    ...
    if message.reference and message.reference.message_id:
        ...
        # - STRONG (for activation on replies to other users) → uses STRONG_DIRECTED_KEYWORDS
        # - GENERAL (for rich context / chain traversal once already activated) → uses GENERAL list
        ...
        explicit_image_reply_keywords = [
            ...
            "edita esta", "edita la", "editá esta", "editá la",
            ...
            "qué ves en esta", "qué ves en la", "analiza esta", "analiza la",
            "describe", "qué ves", "cuéntame de esta", "qué hay en esta",
            "qué ves aquí", "qué ves en la foto", "qué ves en la imagen",
            "explica esta", "explica la", "explica esto", "describe esta", "describe la",
            # Common casual references to the image being replied to
            "qué es esto", "esto qué es", "qué es esta", "mira esta", "mira la", "mira esto",
            "opina de esta", "qué opinas de esta", "qué piensas de esta", "esta imagen", "esta foto",
            # Reference to previous / bot-generated image (critical for text-URL case)
            "esa imagen", "esas imágenes", "la imagen", "esa foto", "la foto",
            ...
            "la que respondí", "la url de la", "esa url",
            ...
            "video con esta foto", "haz video de la que respondí",
        ]
    ...
    # - Mention in current message → always wake
    # - Reply directly to one of *our* previous messages → always wake
    # - Reply to *someone else* → only wake on *very strong* directed signals:
    ...
    #
    # The broad GENERAL_REPLY_INQUIRY_KEYWORDS ("esto", "el anterior", "qué opinas", etc.)
...
async def _harvest_vision_images(
    ...
    and wake the bot via mention or strong directed signals — even if they didn't use one of the
    explicit "qué ves en esta / edita esta" phrases.
    ...
    # but the exact visual keywords were not used → explicit_visual was False.
    ...
    if referenced and is_reply_continuation:
        # Attachment images from referenced — include for any activated reply.
        ...
        # Text-based extraction (bot image links in the referenced text) — keep stricter.
...
async def _invoke_groksito(
    ...
    is_mentioned: bool = False,       # Direct mention of the bot (used to decide recent conversation context injection)
) -> None:
    """
    ...
    # No channel history by default. No custom memory (maximum nativeness / "let Grok be Grok").
    ...
    try:
        ...
        # DIRECT_DELIVERY_PERFORMED is an object() sentinel — use identity check.
        ...
        await safe_reply(message, "Ocurrió un error procesando tu mensaje. Intenta de nuevo.", mention_author=False)
