...
async def call_grok_for_groksito(
    ...
    try:
        ...
        # Proactive summarization — disabled by default (maximum nativeness).
        ...
        # Context injection: high-prio [R:] + deeper reply_chain_contexts (text ancestors for links/referents) on addressed turns. See build_responses_input.
        ...
        # Explicit video intent — used as Python-level guard for offering/allowing generate_video
        ...
        # about whether to fetch context, activate an existing skill, create a new one, edit an existing one,
        # or answer directly — replacing much of the previous hardcoded decision/prompt + counting logic.
        # The offer is gated by a cheap pre-filter so we stay token-efficient and don't turn
        # normal chat into an agent loop.
        ...
        # (casual/minimal/image_gen get none — major token saver).
        #
        ...
        # There is ZERO heuristic that decides whether the *model should call* search — that is 100% Grok
        ...
        if (
            ...
            try:
                ...
                if summary:
                    forced_recent = format_recent_context_block(summary)
                    # Re-inject as an extra system message (the input list is already built)
                    initial_input.append({"role": "system", "content": forced_recent})
                    logger.info(f"{cid_p}[CONTEXT] Forced recent context due to decision layer")
            except Exception:
                pass
        ...
        def _is_direct_delivery_success(result_str: str, tool_name: str) -> bool:
            ...
            if "delivered directly" in lowered or "sent directly" in lowered or "success:" in lowered or "policy blocked" in lowered:
                logger.info(f"{cid_p}[LLM] Direct delivery detected for tool '{tool_name}' — suppressing final text reply")
                return True
            return False
        ...
        for round_num in range(1, max_tool_rounds + 1):
            ...
            if not client_tool_outputs:
                logger.info(f"{cid_p}[LLM] Round {round_num}: no more client-side tool calls — conversation complete.")
                break
            ...
            if direct_delivery_performed:
                logger.info(f"{cid_p}[LLM] Round {round_num}: direct delivery performed — short-circuiting (skip sending tool results back + no further rounds). Natural + cheap.")
                break
            ...
            try:
                ...
                logger.warning(f"{cid_p}[LLM] Continuation with previous_response_id failed: {continue_err} — stopping tool loop")
                break
        ...
        if image_urls:
            ...
            return (
                ...
                "El servicio de visión de Grok está teniendo dificultades con este archivo en este momento. "
        ...
        if isinstance(e, (APITimeoutError, APIConnectionError)) or "timeout" in str(e).lower() or "connection" in str(e).lower():
            return "Tuve un problema de conexión con Grok. Intenta de nuevo en unos segundos."

        # Server errors after retries
        if isinstance(e, APIError):
            status = getattr(e, "status_code", None)
            if status and 500 <= status < 600:
                return "El servicio de Grok está teniendo problemas temporales (5xx). Intenta de nuevo en un momento."
            if status in (401, 403):
                ...
                try:
                    ...
                    if settings.auth_prefers_oauth or settings.using_oauth:
                        hint = " (OAuth token may be invalid/expired or tier-restricted — try `python -m src.groksito_discord --login-oauth` or switch to XAI_API_KEY)"
                except Exception:
                    pass
                return f"Problema de autenticación con Grok (401/403).{hint}"
